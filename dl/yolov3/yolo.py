...
class YOLO(object):
    _defaults = {
        "model_path"        : 'model_data/tube_testStrip_classes_1.pth',
    ...
    def detect_image(self, image):
        ...
        # 打开文件, 准备写入 这里的文件名字要注意一下
        file_name = './predict_result/pos/%s_obj.txt' % (image.filename[4:])
        f = open(file_name, 'w')

        # table.txt当中存档文件的列表
        f2 = open("./predict_result/pos/table.txt", 'a')
        f2.write(file_name)
        f2.write(" ")
        f2.close()
        for i, c in enumerate(top_label):
            ...
            # 将预测出来的内容存到文件中
            f.write("%s %s %d %d %d %d" % (image.filename, label, top, left, bottom, right))
            f.write('\n')
