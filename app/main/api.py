...
# 常量定义
UPLOAD_DIR = "../../static/"
AK = "NGc-bTDt5Vcq410WMyNDA-hLbNyJLNs7BQevMhaB"
SK = "U8w9h0aAztjBNA6jNUlo34FR9lU0b81N6XBdOQbs"
#QINIU_URL = "http://image.ruankun.xyz/"
#QINIU_URL = "http://rr51xpasn.hn-bkt.clouddn.com/"
QINIU_URL = "http://aiassay.fun/"
        ...
        if type(openid) != int:
            ...
            if user is None:
                ...
                return jsonify(code=1, newUser=True, message="login succeed!", id=user.id,
                               token=access_token)#bytes.decode(access_token)
            # 注册过了,登录成功
            access_token = iecuserService.getTokenAndSaveToRedis(user)
            # print(type(user.id))
            # print(type(access_token))
            return jsonify(code=1, message="login succeed!", id=user.id, token=access_token)#bytes.decode(access_token)
        else:
...
def saveImage():
    ...
    try:
        ...
        openid = iecuserService.getOpenidInRedis(request.headers.get("token"))
        user = iecuserService.getUserByOpenid(openid)
        image = iecImageService.saveByUrl(os.path.join(UPLOAD_DIR, filename), user.id, remark)
        return jsonify(code=1, message='upload success', image=RKJsonEncoder.imagetodict(image)) 
...
def process1():
    ...
    # filename = downloadImg(image.url, image.remark, imgpath)  # 1234.jpg
    ...
    if int(obj_num) != int(number):
        ...
        if int(isPH_flag):
            ...
            try:
                ...
                result = yolov3Api.saveProcessWithoutConcentration1(user.id, imageid, image.remark, number, remark)
                
                if result != None:
                    print(result)
                    return jsonify(code=1, message='success!', phValue=result, imageid=imageid)
                else:
...
def downloadImg(url, name, path):
    # 改为从本地存储取
    try:
        image = Image.open("%s" % (url))
        image.save(os.path.join(path, name+".jpg"))
        return (name)
    except urllib.error.URLError as e:
        print(e)
